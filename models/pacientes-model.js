...
class pacientes {
  ...
  static async buscarCedula(cedulaValor) {
    return new Promise((resolve, reject) => {
      try {
        const paciente = laboratorio.pacientes.find(
      ...
      try {
        ...
        const nuevoPaciente = {
          ...
          email: pacienteData.email
            ? pacienteData.email.trim().toLowerCase()
            : "",
          direccion: pacienteData.direccion
            ? pacienteData.direccion.trim()
            : "",
    ...
    return new Promise((resolve, reject) => {
      try {
        if (!cedulaActual) {
          reject(
            new Error(
              "La cédula actual es obligatoria para actualizar un paciente",
            ),
          );
          return;
        }
        ...
        if (pacienteData.cedula && pacienteData.cedula !== cedulaActual) {
          const cedulaRegex = /^V-\d{6,8}$/;
          if (!cedulaRegex.test(pacienteData.cedula)) {
            reject(
              new Error("La nueva cédula debe tener el formato 8 digitos"),
            );
            return;
          }
          ...
          const existeOtraCedula = laboratorio.pacientes.some(
            (p, index) =>
              p.cedula === pacienteData.cedula && index !== pacienteIndex,
          );

          if (existeOtraCedula) {
            reject(
              new Error(
                `La cédula ${pacienteData.cedula} ya está registrada en otro paciente`,
              ),
            );
            return;
          }
        ...
        const pacienteActualizado = {
          ...
          nombre: pacienteData.nombre
            ? pacienteData.nombre.trim()
            : laboratorio.pacientes[pacienteIndex].nombre,
          apellido: pacienteData.apellido
            ? pacienteData.apellido.trim()
            : laboratorio.pacientes[pacienteIndex].apellido,
          telefono: pacienteData.telefono
            ? pacienteData.telefono.trim()
            : laboratorio.pacientes[pacienteIndex].telefono,
          email: pacienteData.email
            ? pacienteData.email.trim().toLowerCase()
            : laboratorio.pacientes[pacienteIndex].email,
          direccion: pacienteData.direccion
            ? pacienteData.direccion.trim()
            : laboratorio.pacientes[pacienteIndex].direccion,
        };
    ...
    return new Promise((resolve, reject) => {
      try {
        if (!cedula || typeof cedula !== "string") {
          reject(
            new Error("La cedula es obligatoria para eliminar un paciente"),
          );
          return;
        }
        ...
        const [pacienteEliminado] = laboratorio.pacientes.splice(
          pacienteIndex,
          1,
        );
  ...
  static async buscarFechas(fechaInicio, fechaFin) {
    return new Promise((resolve, reject) => {
      try {
        const fechaRegex = /^\d{4}-\d{2}-\d{2}$/;
  ...
  static async buscarId(idEnv) {
    console.log("si");
    const juegoEncontrado = laboratorio.pacientes.find((p) => {
      const valor = p.id.startsWith(idEnv);
      if (valor === true) {
        return valor;
      }
    });
    return juegoEncontrado || null;
  }
